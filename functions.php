...
function getEmployee() {
	...
	// the query
	$query = "SELECT * FROM emp ORDER BY name ASC";
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '
			    <tr>
				    ...
				    <td>'.$row["email"].'</td>
				    <td>'.$row["phone"].'</td>
				    <td><a href="emp-edit.php?id='.$row["id"].'" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a> <a data-emp-id="'.$row['id'].'" class="btn btn-danger btn-xs delete-emp"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></td>
			    </tr>
		    ';
		}
